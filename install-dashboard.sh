...
download_nezha() {
    DOWNLOAD_LINK="https://github.com/kandyhe/nezha-freebsd/releases/latest/download/dashboard"
    if ! wget -qO "$INSTALLER_FILE" "$DOWNLOAD_LINK"; then
        echo 'error: Download failed! Please check your network or try again.'
        return 1
    fi
    curl -s https://api.github.com/repos/kandyhe/nezha-freebsd/releases/latest | jq -r '.tag_name' > ${WORKDIR}/VERSION
    return 0
}
